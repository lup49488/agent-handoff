...
def run_trial(task, snapshot, arm, target, replicate, destination, *, model="unrecorded", target_version="unrecorded", wall_seconds=900, launch=False):
    ...
    if launch:
        ...
        try:
            proc = subprocess.run(adapter.exec_argv(prompt), cwd=trial.project, stdin=subprocess.DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding="utf-8", errors="replace", timeout=wall_seconds, check=False)
...
def main(argv: Sequence[str] | None = None):
    ...
    p.add_argument("--model", default="unrecorded"); p.add_argument("--target-version", default="unrecorded"); p.add_argument("--wall-seconds", type=int, default=900); p.add_argument("--launch", action="store_true")
    a=p.parse_args(argv)
